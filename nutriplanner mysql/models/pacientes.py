...
from db.dao import PacienteDAO
...
class Paciente:
    def __init__(self, nome, data_nasc, sexo, gravidez, etnia, profissao, telefone, cpf, email,
                 ...
                 alcool, meta_1, meta_2, meta_3, peitoral, axilar_media, triceps, subescapular, abdomen, suprailiaca, coxa):
        self.nome = nome
    ...
    def calcular_tudo(self):
        ...
        self.percentual_gordura = calc.calcular_percentual_gordura(self)
    ...
    def from_dict(data):
        ...
        p = Paciente(
            ...
            data.get("peitoral"), data.get("axilar_media"), data.get("triceps"),
            data.get("subescapular"), data.get("abdomen"), data.get("suprailiaca"), data.get("coxa")
)
        ...
        p.percentual_gordura = data.get("percentual_gordura", None)
...
class SistemaCadastro:
    ...
    @staticmethod
    def _coletar_lista(titulo: str) -> list:
        print(f" {titulo}: ")
        itens = []
        i = 1
        while True:
            entrada = input(f" {i}- ")
            if entrada == "":
                return itens if itens else ["Não se aplica"]
            itens.append(entrada.capitalize())
            i += 1
    ...
    def cadastro_paciente(self):
        ...
        # ------------- OBJETIVO -----------------
        objetivo = questionary.select("Objetivo principal: ", choices=["Emagrecimento", "Manutenção", "Hipertrofia"], qmark="").ask()

        # ------------ HISTÓRICO DE SAÚDE ------------
        doencas = self._coletar_lista("Doenças diagnosticadas")
        medicamentos = self._coletar_lista("Medicamentos contínuos")
        alergias = self._coletar_lista("Alergias")
        intolerancias = self._coletar_lista("Intolerâncias")
        ...
        atividade_fisica = questionary.select("Nível de atividade física: ", choices=[
            ...
            "Trabalho fisicamente ativo (ex: estoquista, operario)"], qmark="").ask()
        ...
        print("\n📐 DOBRAS CUTÂNEAS (deixe em branco se não aplicável)")
        def pedir_dobra(nome):
            while True:
                val = input(f" {nome} (mm): ").strip()
                if val == "":
                    return None
                try:
                    v = float(val)
        ...
        peitoral = pedir_dobra("Peitoral")
        axilar_media = pedir_dobra("Axilar média")
        triceps = pedir_dobra("Tríceps")
        subescapular = pedir_dobra("Subescapular")
        abdomen = pedir_dobra("Abdômen")
        suprailiaca = pedir_dobra("Suprailíaca")
        coxa = pedir_dobra("Coxa")
        ...
        calc.FacadeCalculos.calcular_tudo(paciente)
    ...
    def salvar(self):
        PacienteDAO.salvar_todos(self.pacientes)

    def carregar(self):
        try:
            self.pacientes = PacienteDAO.carregar_todos()
            print("📂 Pacientes carregados com sucesso do MySQL!")
        except Exception as e:
            print(f"(!) Nenhum dado encontrado ou erro ao carregar: {e}")
    ...
    def visualizar_paciente(self):
        ...
        print("-" * 60)
        dobras = {
            "Peitoral": paciente_selecionado.peitoral,
            "Axilar média": paciente_selecionado.axilar_media,
            "Tríceps": paciente_selecionado.triceps,
            "Subescapular": paciente_selecionado.subescapular,
            "Abdômen": paciente_selecionado.abdomen,
            "Suprailíaca": paciente_selecionado.suprailiaca,
            "Coxa": paciente_selecionado.coxa,
        }
        for nome, val in dobras.items():
            if val is not None:
                print(f"  {nome}: {val} mm")
            else:
    ...
    def editar_paciente(self):
        ...
        while True:
            ...
            if escolha == "Nome":
                ...
                paciente.objetivo = questionary.select(
                    "Objetivo principal:",
                    choices=["Emagrecimento", "Manutenção", "Hipertrofia"],
                    qmark=""
                ).ask()
                ...
                paciente.doencas = self._coletar_lista("Nova lista")
                ...
                for d in paciente.medicamentos:
                    print(f"  - {d}")
                print("\nDigite novamente a lista (ENTER para finalizar)")
                paciente.medicamentos = self._coletar_lista("Nova lista")
                ...
                for d in paciente.alergias:
                    print(f"  - {d}")
                print("\nDigite novamente a lista (ENTER para finalizar)")
                paciente.alergias = self._coletar_lista("Nova lista")
                ...
                paciente.intolerancias = self._coletar_lista("Nova lista")
                ...
                paciente.atividade_fisica = questionary.select(
                    ...
                    choices=[
                        ...
                        "Intenso (5-6x por semana)",
                        "Atleta/treino diário",
                        "Trabalho fisicamente ativo (ex: estoquista, operario)"
                   ],
                    qmark=""
                ).ask()
            ...
            elif escolha == "Dobras cutâneas":
                def pedir_dobra(nome, atual):
                    val = input(f" {nome} (mm) [atual: {atual}]: ").strip()
                    if val == "":
                        return atual
                    try:
                        v = float(val)
                ...
                paciente.peitoral = pedir_dobra("Peitoral", paciente.peitoral)
                paciente.axilar_media = pedir_dobra("Axilar média", paciente.axilar_media)
                paciente.triceps = pedir_dobra("Tríceps", paciente.triceps)
                paciente.subescapular = pedir_dobra("Subescapular", paciente.subescapular)
                paciente.abdomen = pedir_dobra("Abdômen", paciente.abdomen)
                paciente.suprailiaca = pedir_dobra("Suprailíaca", paciente.suprailiaca)
                paciente.coxa = pedir_dobra("Coxa", paciente.coxa)
    ...
    def deletar_paciente(self):
        ...
        if confirmar == "Sim":
            ...
            if cpf_para_remover:
                PacienteDAO.deletar_paciente(cpf_para_remover)
